...
error:
        self->state = ARGPARSE_PARSE_ERROR;
...
{
    if (self->usages) {
        const char *const * usage = self->usages;
    ...
    for (; options->type != ARGPARSE_OPT_END; options++) {
        ...
        if (options->type == ARGPARSE_OPT_GROUP) {
            fputc('\n', stderr);
            fprintf(stderr, "%s", options->help);
            fputc('\n', stderr);
            continue;
        }
        pos = fprintf(stderr, "    ");
        if (options->short_name) {
            pos += fprintf(stderr, "-%c", options->short_name);
        }
        if (options->long_name && options->short_name) {
            pos += fprintf(stderr, ", ");
        }
        if (options->long_name) {
            pos += fprintf(stderr, "--%s", options->long_name);
        }
        if (options->type == ARGPARSE_OPT_INTEGER) {
            pos += fprintf(stderr, "=<int>");
        } else if (options->type == ARGPARSE_OPT_FLOAT) {
            pos += fprintf(stderr, "=<flt>");
        } else if (options->type == ARGPARSE_OPT_STRING) {
