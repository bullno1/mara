...
// #define x_size(u) should point to storage for the size of the hash table
...
#define ROBINHOOD_HASH_FOREACH(tbl, u, i) \
       _ROBINHOOD_HASH_FOREACH(tbl, u, i)
...
#define _ROBINHOOD_HASH_DEL(tbl, u, key) { \
    ...
    for(;;) { \
        if (tbl##_isnil(u, _rh_i)) { \
            ...
            _ROBINHOOD_HASH_TYPICAL_INCREMENT({_shrunk = 0;tbl##_removefailed(u, key);}, tbl##_n_elem(u)) \
        } \
    } \
    /* Backshift */ \
...
#define _ROBINHOOD_HASH_FOREACH(tbl, u, i) \
    for(size_t i = 1; i < tbl##_n_elem(u); ++i) \
        if(!(tbl##_isnil(u, i)))
